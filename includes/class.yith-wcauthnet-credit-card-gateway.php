...
if ( ! class_exists( 'YITH_WCAUTHNET_Credit_Card_Gateway' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class YITH_WCAUTHNET_Credit_Card_Gateway extends WC_Payment_Gateway_CC {
		...
		public function __construct() {
			$this->id                 = YITH_WCAUTHNET::$gateway_id;
			...
			$this->show_cards        = $this->get_option( 'show_cards' ) === 'yes';
		...
		public function init_form_fields() {
			$this->form_fields = apply_filters( 'yith_wcauthnet_credit_card_gateway_options', array(
				...
				'show_cards'       => array(
					'title'       => __( 'Show credit card icons', 'yith-woocommerce-authorizenet-payment-gateway' ),
					'type'        => 'checkbox',
					'description' => __( 'Enable to show credit card icons in checkout page', 'yith-woocommerce-authorizenet-payment-gateway' ),
					'default'     => 'yes'
				),
				'card_types'       => array(
		...
		public function get_icon() {
			...
			if ( $this->show_cards && ! empty( $this->card_types ) ) {
				...
				foreach ( $this->card_types as $card_type ) {
					...
					if ( file_exists( YITH_WCAUTHNET_DIR . 'assets/images/icons/credit-cards/' . strtolower( $card_type ) . '.png' ) ) {
						$icon .= '<img style="float: right; max-width: 40px; margin-left: 10px;" src="' . esc_url( WC_HTTPS::force_https_url( YITH_WCAUTHNET_URL ) . '/assets/images/icons/credit-cards/' . strtolower( $card_type ) . '.svg' ) . '" alt="' . esc_attr( strtolower( $card_type ) ) . '" />';
					}
		...
		/**
		 * Return relay url for the redirect mode
		 *
		 * @return string Relay url
		 * @since 1.2.1
		 */
		public function get_relay_url() {
			return esc_url( add_query_arg( 'wc-api', $this->id, user_trailingslashit( home_url() ) ) );
		}
